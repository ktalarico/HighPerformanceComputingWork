...
int main(int argc, char* argv[]){
  ...
  int numRot, blockSize=32, streamNum=1, streamSize;
  double theta = 0.;
  x = atoll(argv[1]);
  y = atoll(argv[2]);
  theta = atof(argv[3]);
  ...
  int numBlocks = ((rotSizex*rotSizey)+(blockSize-1))/blockSize;
  int streamBlk = (numBlocks+(streamSize-1))/streamSize;
  ...
  cudaMalloc((void **)&(d_centered.r), streamSize*blockSize*sizeof(uint8_t));
#ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif

  cudaMalloc((void **)&(d_centered.g), streamSize*blockSize*sizeof(uint8_t));
#ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif

  cudaMalloc((void **)&(d_centered.b), streamSize*blockSize*sizeof(uint8_t));
#ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
  ...
  cudaMalloc((void **)&(d_result.r), streamNum*streamSize*blockSize*sizeof(uint8_t));
#ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
  
  cudaMalloc((void **)&(d_result.g), streamNum*streamSize*blockSize*sizeof(uint8_t));
#ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif

  cudaMalloc((void **)&(d_result.b), streamNum*streamSize*blockSize*sizeof(uint8_t));
#ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
  d_result.x = rotSizex;
  ...
  for(int k=0; k<numRot; k++){
    ...
    for(int l=0; l<streamBlk; l++){
      if(l*streamSize*blockSize+streamSize*blockSize<=rotSizex*rotSizey){
	cudaMemcpyAsync((void *)&(d_centered.r[l%streamNum*streamSize*blockSize]), (void *)&(centered.r[l*streamSize*blockSize]), streamSize*blockSize*sizeof(uint8_t), cudaMemcpyHostToDevice, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
  
  cudaMemcpyAsync((void *)&(d_centered.g[l%streamNum*streamSize*blockSize]), (void *)&(centered.g[l*streamSize*blockSize]), streamSize*blockSize*sizeof(uint8_t), cudaMemcpyHostToDevice, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
  
  cudaMemcpyAsync((void *)&(d_centered.b[l%streamNum*streamSize*blockSize]), (void *)&(centered.b[l*streamSize*blockSize]), streamSize*blockSize*sizeof(uint8_t), cudaMemcpyHostToDevice, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
      }else{
	cudaMemcpyAsync((void *)&(d_centered.r[l%streamNum*streamSize*blockSize]), (void *)&(centered.r[l*streamSize*blockSize]), (rotSizex*rotSizey-l*streamSize*blockSize)*sizeof(uint8_t), cudaMemcpyHostToDevice, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
  
  cudaMemcpyAsync((void *)&(d_centered.g[l%streamNum*streamSize*blockSize]), (void *)&(centered.g[l*streamSize*blockSize]), (rotSizex*rotSizey-l*streamSize*blockSize)*sizeof(uint8_t), cudaMemcpyHostToDevice, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
  
  cudaMemcpyAsync((void *)&(d_centered.b[l%streamNum*streamSize*blockSize]), (void *)&(centered.b[l*streamSize*blockSize]), (rotSizex*rotSizey-l*streamSize*blockSize)*sizeof(uint8_t), cudaMemcpyHostToDevice, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
      }

      rotateImage<<<streamSize, blockSize, 0, streams[l%streamNum]>>>(d_centered, d_result, theta, l%streamNum, streamSize);
#ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif

  if(l*streamSize*blockSize+streamSize*blockSize<=rotSizex*rotSizey){
	cudaMemcpyAsync((void *)&(result.r[l%streamNum*streamSize*blockSize]), (void *)&(d_result.r[l*streamSize*blockSize]), streamSize*blockSize*sizeof(uint8_t), cudaMemcpyDeviceToHost, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
  
  cudaMemcpyAsync((void *)&(result.g[l%streamNum*streamSize*blockSize]), (void *)&(d_result.g[l*streamSize*blockSize]), streamSize*blockSize*sizeof(uint8_t), cudaMemcpyDeviceToHost, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
  
  cudaMemcpyAsync((void *)&(result.b[l%streamNum*streamSize*blockSize]), (void *)&(d_result.b[l*streamSize*blockSize]), streamSize*blockSize*sizeof(uint8_t), cudaMemcpyDeviceToHost, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
      }else{
	cudaMemcpyAsync((void *)&(result.r[l%streamNum*streamSize*blockSize]), (void *)&(d_result.r[l*streamSize*blockSize]), (rotSizex*rotSizey-l*streamSize*blockSize)*sizeof(uint8_t), cudaMemcpyDeviceToHost, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
  
  cudaMemcpyAsync((void *)&(result.g[l%streamNum*streamSize*blockSize]), (void *)&(d_result.g[l*streamSize*blockSize]), (rotSizex*rotSizey-l*streamSize*blockSize)*sizeof(uint8_t), cudaMemcpyDeviceToHost, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
  
  cudaMemcpyAsync((void *)&(result.b[l%streamNum*streamSize*blockSize]), (void *)&(d_result.b[l*streamSize*blockSize]), (rotSizex*rotSizey-l*streamSize*blockSize)*sizeof(uint8_t), cudaMemcpyDeviceToHost, streams[l%streamNum]);
  #ifdef DEBUG
  std::cout<<__LINE__<<" Error: "<<cudaGetErrorString(cudaGetLastError())<<std::endl;
#endif
      }
